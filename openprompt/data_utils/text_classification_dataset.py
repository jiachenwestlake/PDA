...
class MnliMultiProcessor(DataProcessor):
    def __init__(self):
        super().__init__()
        self.labels = ["contradiction", "entailment", "neutral"]
    ...
    def get_examples(self, data_dir, split):
        ...
        with open(os.path.join(data_dir, '{}.txt'.format(split)), 'r') as fin:
            for idx, line in enumerate(fin.readlines()):
                ...
                text_b = text_b.strip().replace('\\', ' ')
                example = InputExample(
                    guid=str(idx), text_a=text_a, text_b=text_b, label=self.labels.index(label))
                examples.append(example)
...
PROCESSORS = {
    ...
    "sst-2": SST2Processor,
    "mnli": MnliProcessor,
    "amazonmulti": AmazonMultiProcessor,
    "mnlimulti": MnliMultiProcessor,
}
